...
export const memory: ModuleDef = {
  ...
  items: [
    ...
    { id: 'mem.reg', name: 'Registers', kind: 'atom', parent: 'mem.onchip', zone: 'The staircase', x: 300, y: 128, hitR: 10, ldx: 46,
      note: '256 KB per SM — per-thread and single-cycle, the only memory the ALU computes from directly. Everything below this line is staging.' },
    { id: 'mem.l1', name: 'Shared memory / L1', kind: 'atom', parent: 'mem.onchip', zone: 'The staircase', x: 300, y: 150, hitR: 10, ldy: 18,
      note: 'Up to 228 KB per SM, software-managed — kernels are written around tiling data through this strip. FlashAttention lives here.' },
    ...
    { id: 'mem.hbm', name: 'HBM', kind: 'atom', parent: 'mem.inpkg', zone: 'The staircase', x: 445, y: 330, hitR: 25,
      note: '80 GB at 3.35 TB/s (H100); 192 GB at ~8 TB/s (Blackwell). Weights and every user’s KV pages fight for this space — "out of memory" means out of these towers.',
    ...
    { id: 'mem.ddr', name: 'Host DRAM (DDR)', kind: 'atom', parent: 'mem.system', zone: 'The staircase', x: 712, y: 396, hitR: 45,
      note: 'The eight slots: terabyte-class capacity at ~0.5 TB/s — an order of magnitude below HBM, and a PCIe hop away from the GPU.' },
    { id: 'mem.nvme', name: 'NVMe SSD', kind: 'atom', parent: 'mem.system', zone: 'The staircase', x: 887, y: 388, hitR: 45,
      note: 'The two sticks: ~14 GB/s, 200× slower than HBM. Checkpoints and datasets live here — a frontier model takes tens of seconds to load, not milliseconds.' },
    { id: 'mem.offload', name: 'Offloading', kind: 'atom', parent: 'mem.system', zone: 'The staircase', x: 716, y: 470, hitR: 30,
      note: 'The dashed path: spill weights or KV pages from HBM to the DIMMs, then to the sticks. The model still runs — but every tick now waits on the slow rungs.' },
    { id: 'mem.cxl', name: 'CXL memory expansion', kind: 'atom', parent: 'mem.system', zone: 'The staircase', x: 887, y: 507, hitR: 45,
      note: 'The add-in card: pools extra DRAM behind a PCIe-like link — capacity for cheap, at a latency the workload has to be able to tolerate.' },

    { id: 'mem.roofline', name: 'Arithmetic intensity', kind: 'atom', zone: 'The physics', x: 172, y: 445, hitR: 50,
      note: 'The roofline itself: below the ridge you’re bandwidth-bound (decode lives here), past it compute-bound (prefill). FLOPs-per-byte decides which wall you hit.',
    ...
    { id: 'mem.wall', name: 'The memory wall', kind: 'atom', zone: 'The physics', x: 335, y: 440, hitR: 40,
      note: 'The two curves pulling apart: compute grows faster than bandwidth every generation — so more of AI becomes a memory problem, not a math problem.' },
